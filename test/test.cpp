...
#define ASSERT_RES(res, msg, id, err) \
	if ((res)) { \
		printf(msg, id, err); \
		throw std::runtime_error("assert on " + std::to_string(__LINE__)); \
	} \
	else (void)0


void server() {
	lsc::LScoket<AF_INET, SOCK_STREAM, 0> binder;

	int res = binder.Bind("0.0.0.0", 6969);
	ASSERT_RES(res, "%d failed to bind: %d", 0, errno);

	res = binder.Listen(2);
	ASSERT_RES(res, "%d failed to listen: %d", 0, errno);

	// we can ditch the binder now and create the client socket

	res = binder.Accept();
	ASSERT_RES(res < 0, "%d failed to accept: %d", 0, errno);

	lsc::LScoket<AF_INET, SOCK_STREAM, 0> client1(res, lsc::EStat_connected);

	res = binder.Accept();
	ASSERT_RES(res < 0, "%d failed to accept: %d", 0, errno);

	lsc::LScoket<AF_INET, SOCK_STREAM, 0> client2(res, lsc::EStat_connected);

	char msg[] = "hello there :P";

	res = client1.Send(msg, strlen(msg));
	ASSERT_RES(res < 0, "%d failed to send: %d", 0, errno);
	...
	ASSERT_RES(res < 0, "%d failed to recv: %d", 0, errno);
...
void client(int id, std::string msg) {
	lsc::LScoket<AF_INET, SOCK_STREAM, 0> conn;

	int res = conn.Connect("127.0.0.1", 6969);
	ASSERT_RES(res, "%d failed to connect: %d", id, errno);

	char conn_recv[256];

	res = conn.Recv(conn_recv, sizeof(conn_recv));
	ASSERT_RES(res < 0, "%d failed to recv: %d", id, errno);

	std::cout << "conn" << id << " got: " << conn_recv << '\n';

	res = conn.Send(msg.c_str(), msg.size());
	ASSERT_RES(res < 0, "%d failed to send: %d", id, errno);
